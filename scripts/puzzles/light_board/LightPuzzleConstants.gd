...
enum PieceType {
	...
	GLASS_BLOCK,
	OPAQUE_BLOCK,
	PLANE_MIRROR_HORIZONTAL,
	PLANE_MIRROR_VERTICAL,
}
...
static func reflect_horizontal(direction: int) -> int:
	var vector := direction_vector(direction)
	return vector_direction(Vector2i(vector.x, -vector.y))


static func reflect_vertical(direction: int) -> int:
	var vector := direction_vector(direction)
	return vector_direction(Vector2i(-vector.x, vector.y))
